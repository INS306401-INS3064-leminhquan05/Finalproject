...
class Phase2Service {
    ...
    public function getSubmissionsForTeacher($teacherId, $classId = null) {
        $sql = "
            ...
            JOIN teacher_assignments ta ON ta.class_id = sub.class_id AND ta.student_id = sub.student_id
                AND ta.teacher_id = :tid AND ta.assignment_status = 'CONFIRMED'
            LEFT JOIN grades g ON g.submission_id=sub.id
            WHERE 1=1
        ";
        $params = ['tid' => $teacherId];
        if ($classId) { $sql .= " AND sub.class_id=:cid"; $params['cid'] = $classId; }
    ...
    public function gradeSubmission($submissionId, $teacherId, $score, $comment) {
        ...
        if ($row = $exist->fetch()) {
            ...
            $ok = $this->db->prepare("INSERT INTO grades (submission_id, teacher_id, score, comment) VALUES (:sid,:tid,:sc,:cm)")
                ->execute(['sid' => $submissionId, 'tid' => $teacherId, 'sc' => $score, 'cm' => $comment]);
        }
        if ($ok) {
