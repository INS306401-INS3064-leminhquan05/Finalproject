...
class PaymentService {
    ...
    public function listPending(int $limit = 100): array {
        ...
        foreach ($rows as &$row) {
            $row['has_schedule_conflict'] = !empty(
                $this->scheduleService->checkStudentScheduleConflict((int)$row['student_id'], (int)$row['class_id'])
            );
            $cnt = $this->db->prepare("
    ...
    public function confirmPayment(array $data, int $adminUserId): array {
        ...
        if ($conflicts) {
            return ['error' => ScheduleService::conflictClassMessage($conflicts, 'học')];
        }
